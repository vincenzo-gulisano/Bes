...
import common.AvgStat;

public class KafkaReceiver {

	public static AvgStat stats;

	public static void main(String args[]) {

		if (args.length != 5) {
			System.out
					.println("Usage:\n\tKafkaReceiver <topic> <broker> <port> <exp duration> <statsFile>");
			return;
		}
		...
		long expDuration = Long.valueOf(args[3]);
		System.out.println("expDuration: " + expDuration);
		String statsFile = args[4];
		System.out.println("statsFile: " + statsFile);

		stats = new AvgStat("", statsFile, true);
		stats.start();

		KafkaReceiver example = new KafkaReceiver();

		// long maxReads = 10000000;
		int partition = 0;

		List<String> seeds = new ArrayList<String>();
		seeds.add(broker);
		try {
			example.run(expDuration, topic, partition, seeds, port);
		} catch (Exception e) {
			System.out.println("Oops:" + e);
			e.printStackTrace();
		}

		System.out.println("writing statistics and exiting");
		stats.stopStats();
		stats.writeStats();
	...
	public void run(long duration, String a_topic, int a_partition,
		...
		while (System.currentTimeMillis() - startTime <= duration * 1000) {
			...
			for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(
				...
				payload.get(bytes);
				String msg = String.valueOf(messageAndOffset.offset()) + ": "
						+ new String(bytes, "UTF-8");
				// System.out.println(msg);

				// Update statistic
				stats.add(Long.valueOf(msg.split(" - ")[1]));
